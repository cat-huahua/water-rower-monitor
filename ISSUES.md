...
| 7 | recovery 段估算有效阻力系数 k_eff（观察用显示，不参与距离） | enhancement | P2 |
...
**已实现（观察用）。** recovery 段用"`1/ω`（∝间隔）线性增长、斜率 = k/I"估出 `kEff`，多桨 EMA 平滑，显示在**暂停页**（`Drag k:`）。**不参与距离**——水阻涡流使其偏噪、且绝对刻度仍需一次已知距离标定，故仅作观察/稳定性参考。`FLYWHEEL_INERTIA`（main.cpp，默认 0.1）只缩放显示值。
