...
```
  ...
  └────────────────┤                    │
                   │                    │
  ┌─ 传感器 ───────┤  (原机3线, HW-487/LM393 类型)
  │  !! 需要5V !!   │  GPIO 5  ── SIG ──┤  蓝线 (数字信号)
  │                │  5Vin    ── VCC   │  红线 (必须5V!)
  │                │  GND     ── GND ──┘  绿线
  └────────────────┤                │
                   │                │
...
## 原机传感器 (HW-487 / LM393 光遮断器, 已测试确认!)

```
线缆颜色       →   ESP32-S3              注意
──────────────────────────────────────────────
红 (VCC)       →   5Vin                  !! 必须 5V, 3.3V 不够 !!
绿 (GND)       →   GND
蓝 (SIG)       →   GPIO 5               数字信号, 遮光=LOW
```

> 传感器内部有 LM393 比较器 + IR LED 限流电阻
> 不需要外接电阻, 直接接就行
> 3.3V 供电 IR LED 太暗, 必须用 5V
